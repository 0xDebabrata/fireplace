...
const Watch = () => {
  ...
  const isConnected = (participant) => {
    return (
      ["Decline", "Error", "Kicked", "Left"].indexOf(participant.status) < 0
    );
  };
  ...
  const handleMute = () => {
    try {
      if (mute) {
        VoxeetSDK.current.conference.mute(
          VoxeetSDK.current.session.participant,
          false
        );
        setMute(false);
        setStatus("listening");
  ...
  const setSpatialEnvironment = () => {
    ...
    VoxeetSDK.current.conference.setSpatialEnvironment(
      scale,
      forward,
      up,
      right
  ...
  const setLocalPosition = () => {
    VoxeetSDK.current.conference.setSpatialPosition(
      VoxeetSDK.current.session.participant,
      {
  ...
  useEffect(() => {
    ...
    getAccessToken()
      ...
      .then(async () => {
        ...
        const conference = await VoxeetSDK.current.conference.create(
          conferenceOptions
        );
        ...
        arr.map((val) => {
          const participant = val[1];
          if (
            participant.id !== VoxeetSDK.current.session.participant.id &&
  ...
  useEffect(() => {
    ...
    if (router.isReady) {
      ...
      ws.current = new WebSocket(
        `${process.env.NEXT_PUBLIC_WEBSOCKET_URL}/${clientId}`
      );
